...
def airport_country_matches(airport : dict , country_code : str) -> bool:
    ...
    try:
        country = pycountry.countries.get(aplha_2=country_code)
...
def get_best_airport_for_countries(country_code : str):
...
def resolve_location_to_iata(location : str) : 
    ...
    for iata , airport in AIRPORTS.items():
        ...
        if score > 0:
            city_mactches.append((score , iata))

    if city_mactches:
        city_mactches.sort(reverse=True)
        return city_mactches[0][1]
...
def format_flight(flight : dict):
    ...
    flight_number = flight.get("number",{}).get("iata") or "Unknown Flight Number"
...
def search_flights(query : str , limit : int = 10):
    if not API_KEY:
        return ("Flight API error: AVIATIONSTACK_API_KEY is missing.\n"
            "Please add this in your .env file:\n"
            "AVIATIONSTACK_API_KEY=your_api_key_here")

    dep_iata , arr_iata = parse_routes(query)
    ...
    try:
        ...
        return f"Flight API Invalid JSON."
